...
function Weather.draw(renderer, state, index, count)
    ...
    renderer:panel(24, 86, 550, 350, renderer:clip(weather.location, 20) .. " / Today's high")
    renderer:weatherIcon(weather.condition, 48, 160, 132)
    renderer:text(weather.high, 208, 164, 5, { bold = true })
    renderer:text(renderer:clip(weather.condition:upper(), 18), 208, 252, 3, { bold = true })
    renderer:text("CURRENT " .. weather.temperature .. "   FEELS " .. weather.apparentTemperature, 208, 314, 1, { color = "GRAY5" })
    ...
    renderer:label("Current", 618, 158)
    renderer:text(weather.temperature, 618, 194, 3, { bold = true })
