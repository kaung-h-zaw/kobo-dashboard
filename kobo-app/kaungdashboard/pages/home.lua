...
function Home.draw(renderer, state, index, count)
    ...
    renderer:text(os.date("%H:%M"), 350, 174, 3, { bold = true })
    ...
    renderer:text(event and event.time or "--:--", 42, 402, 2, { bold = true })
    renderer:text(renderer:clip(event and event.title or "No events", 27), 42, 438, 2, { bold = true })
    renderer:line(42, 474, 442, 1, "GRAY8")
    renderer:label("Next reminder", 42, 486, { scale = 1 })
    renderer:text(renderer:clip(reminder and reminder.title or "No reminders", 27), 42, 510, 2, { bold = true })
    renderer:text(reminder and reminder.due and reminder.due:upper() or "NO DUE TIME", 42, 546, 1, { color = "GRAY5" })
