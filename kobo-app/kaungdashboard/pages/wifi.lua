...
local function draw_network(renderer, x, width, band, ssid, asset)
    ...
    renderer:label("Network", x + 24, 500, { scale = 1 })
    renderer:text(ssid, x + 24, 526, 3, { bold = true })
    renderer:label("Scan to connect", x + 24, 586, { scale = 1 })
    renderer:text("SAVED GUEST ACCESS", x + 24, 616, 1, { color = "GRAY6" })
end
